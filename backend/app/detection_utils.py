...
CLASS_NAMES = {0: 'helmet', 1: 'vest'}  # Match actual model classes
...
def draw_boxes(frame, results, conf_thres=0.35):
    ...
    for r in results: # results from model(frame)
        ...
        for b in boxes:
            ...
            # Map model class to correct names (model was trained with swapped labels)
            if cls == 0:  # helmet in model but actually detects vest
                label = 'vest'
            elif cls == 1:  # safety_vest in model but actually detects helmet
                label = 'helmet'
            else:
                label = f'class_{cls}'
...
def check_ppe_for_workers(detections, current_employee=None):
    ...
    if missing_ppe and (current_time - check_ppe_for_workers.last_notification) > 30:
        ...
        try:
            with open("data/logs/ppe_violations.log", "a") as f:
